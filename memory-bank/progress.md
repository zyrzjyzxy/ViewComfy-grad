...
- [x] 完善了管理员工作台界面，根据需求文档进行了优化和补充。
  - ✅ 创建用户页面 /admin/users/new/page.tsx（创建新用户）
  - ✅ 完善生成记录管理页面，添加批量导出功能
  - ✅ 优化侧边栏导航，添加管理后台子菜单显示（统计数据、用户管理、记录管理）
  - ✅ 确保管理员工作台与用户工作台有明显界面区分
  - ✅ 实现所有管理功能的完整 UI 和交互逻辑
  - ✅ 修改 AuthContext 登录逻辑，根据用户角色跳转到不同页面
    - 管理员登录后跳转到 /admin
    - 普通用户登录后跳转到 /editor
